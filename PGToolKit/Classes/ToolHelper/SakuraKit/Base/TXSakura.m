...
NSString *const kTXSakuraArgNaviBarBackgroundImage = @"com.tingxins.sakura.arg.backgroundImage";
...
- (TXSakura2DBarMetricsBlock)tx_sakuraImageForBarMetricsBlockWithName:(NSString *)name{
    return ^TXSakura *(NSString *path, UIBarMetrics barMetrics){
        return [self send2DMsgObjectAndIntWithName:name keyPath:path integ:barMetrics selTail:kTXSakura2DBarMetricsSELTail argType:kTXSakuraArgNaviBarBackgroundImage valueBlock:^NSObject *(NSString *keyPath) {
            return [TXSakuraManager tx_imageWithColor:keyPath];
        }];
    };
}
